...
ALI_TONGYI_3 = "qwen3-235b-a22b"#目前在兼容性上还有些问题，云帆老师的免费额度最近到期时间：2025-10-26
ALI_TONGYI_REASONER_MODEL = "qwq-plus"  #云帆老师的免费额度最近到期时间：2025-09-22
ALI_TONGYI_EMBEDDING_MODEL = "text-embedding-v3"
ALI_TONGYI_EMBEDDING_MODEL_V4 = "text-embedding-v4"#云帆老师的免费额度最近到期时间：2025-12-1
...
def get_lc_o_tencent_model_client(model=TENCENT_HUNYUAN_TURBO_MODEL, temperature=0.7, verbose=False, debug=False):
    """通过LangChain使用腾讯大模型"""
    return get_lc_o_model_client(api_key=os.getenv(TENCENT_HUNYUAN_API_KEY_OS_VAR_NAME), base_url=TENCENT_HUNYUAN_URL
                                 , model=model, temperature=temperature, verbose=verbose, debug=debug)
...
def get_lc_baichuan_embeddings():
    '''
...
def get_lc_tencent_embeddings():
    '''
    通过LangChain获得一个腾讯嵌入模型的实例，
    腾讯模型需要单独在腾讯云上再申请hunyuan_secret_id和hunyuan_secret_key
    :return: 腾讯嵌入模型的实例
    '''
...
def get_lc_a_t_mix_clients():
    '''
    以OpenAI兼容的方式，混合产生阿里的大模型客户端和腾讯嵌入模型的客户端
    :return: 阿里的大模型客户端和腾讯嵌入模型的客户端
    '''
    return get_lc_o_ali_model_client(),get_lc_tencent_embeddings()

def get_lc_ali_rerank(top_n=3):
    '''
