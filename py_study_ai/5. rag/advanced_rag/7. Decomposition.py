...
result = chain.invoke({"question":"番茄炒蛋怎么制作？"})
print(result)
print("-------------完成测试大模型对问题的拆解--------------")

DEFAULT_SUB_QUESTION_PROMPT = PromptTemplate(
    input_variables=["question", "sub_question", "documents"],
    template="""要解决主要问题{question}，需要先解决子问题{sub_question}。以下是为支持您的推理而提供的参考文档：{documents}。请直接给出当前子问题的答案。不需要额外内容。""",
)
...
decomposition_docs = decompositionQueryRetriever.invoke("新手如何制作番茄炒蛋？")
print("-------------检索到的文档（拆解后）--------------")
