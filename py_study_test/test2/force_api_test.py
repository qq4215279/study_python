...
class ForceModule(threading.Thread):
    ...
    def send_msg_and_receive(self, protocol_name: str, params: list):
        return self.task.self.task.send_msg_and_receive(protocol_name, params)
...
class WarOrderForceModule(ForceModule):
    ...
    def _do_before_test(self):
        # 增加道具
        self.add_command("ReqGiveMeItems", [{"1006": 2000000, "1108": 200000, "1109": 200000, "1110": 100000}, helper.KEY, self.playerId])
        pass
    ...
    def handle_receive(self, protocal_name: str, res: dict):
        ...
        elif protocal_name.find("ResGetSummerTourInfo") != -1:
            type = self.__get_draw_summer_tour_type(res)
            if type != -1:
                # 4. 请求寻访  寻访类型: 1: 阳光海滩; 2: 泳池派对    次数
                # self.add_command("ReqDrawSummerTour", [random.randint(1, 2), random.choice((1, 10))])
                self.add_command("ReqDrawSummerTour", [type, random.choice((1, 10))])



    def __get_draw_summer_tour_type(self, res_dist: dict):
        typeList = list()
        for summerTourInfo in res_dist["summerTourInfos"]:
            for tourInfo in summerTourInfo["configTourInfo"]:
                if not tourInfo["isBirght"]:
                    typeList.append(summerTourInfo["type"])
                    break
...
class AllRoomAllFishForceModule(ForceModule):
    ...
    def _do_before_test(self):
        ...
        if self.maxCannonMultiple < 500000:
            # 升级炮倍
            self.send_msg_and_receive("ReqOBUnlockCannonLv", [50000, True])
            # time.sleep(1)
            # 切换到50000炮倍
