...
关键字：
1. async 
    用于定义一个协程函数。协程函数的调用不会立即执行，而是返回一个协程对象。协程函数的定义方式是在函数前加上 async 关键字。
2. await
    用于在协程中调用另一个协程。它会暂停当前协程的执行，直到被 await 的协程完成，并返回结果。
    await 只能在 async 定义的协程内部使用。使用 await 使得代码在等待某个操作完成时不会阻塞整个程序的执行。
...
5. 其他实用函数
    5.1. asyncio.run_until_complete()  在事件循环中运行一个协程，直到它完成。eg: loop.run_until_complete(main())  => 方法已过期，推荐使用 asyncio.run(main())
...
def sync_fetch_data():
    print("start sync fetching data2...")
    # time.sleep(2)  # 模拟网络请求
    return "fetched sync finish2"
